use crate::{material::ripple::Ripple, util::ColorConversion, THEME};
use freya::prelude::*;

#[derive(Default, PartialEq, Eq)]
enum FabState {
    #[default]
    Idle,
    Hovering,
    Pressed,
}

#[derive(Props, PartialEq, Clone)]
pub struct FabProps {
    pub extended: bool,
    pub onclick: EventHandler,
    pub icon: String,
    pub label: String,
}

#[component]
#[allow(clippy::needless_pass_by_value)]
pub fn FAB(props: FabProps) -> Element {
    let platform = use_platform();
    let (node_ref, size) = use_node();
    
    let mut state = use_signal(FabState::default);

    let animation = use_animation(move |ctx| {
        (
            ctx.with(AnimNum::new(0.0, size.area.width().max(48.0)).time(300)),
            ctx.with(AnimNum::new(0.0, 1.0).time(300)),
        )
    });

    let onmousedown = move |e: MouseEvent| {
        e.stop_propagation();

        state.set(FabState::Pressed);
    };

    let onmouseleave = move |e: MouseEvent| {
        e.stop_propagation();

        state.set(FabState::Idle);

        platform.set_cursor(CursorIcon::default());
    };

    let onmouseenter = move |e: MouseEvent| {
        e.stop_propagation();

        state.set(FabState::Hovering);

        platform.set_cursor(CursorIcon::Pointer);
    };

    let onclick = move |e: MouseEvent| {
        e.stop_propagation();

        props.onclick.call(());

        state.set(FabState::Idle);
    };

    use_effect(use_reactive(&props.extended, move |extended| {
        if extended {
            animation.start();
        } else {
            animation.reverse();
        }
    }));

    let (width, opacity) = animation.get();
    let (width, opacity) = (width.read().as_f32(), opacity.read().as_f32());

    println!("{width}");

    rsx! {
        rect {
            height: "56",
            width: "calc(56 + {width})",
            corner_radius: "16",
            main_align: "center",
            background: "{THEME.primary_container}",
            padding: "16",
            direction: "horizontal",

            onmousedown,
            onmouseenter,
            onmouseleave,
            onclick,

            rect {
                position: "absolute",
                position_top: "-16",
                position_left: "-16",
                height: "56",
                width: "{width}",

                Ripple {
                    color: "{THEME.on_primary_container.to_rgb()}",
                    height: "56",
                    width: "{width}",
                }
            }

            label {
                width: "24",
                height: "24",
                color: "{THEME.on_primary_container}",
                font_size: "24",

                {props.icon.as_str()}
            }

            rect {
                overflow: "clip",
                width: "{width}",
                height: "100%",

                rect {
                    // width: "auto",
                    height: "100%",
                    opacity: "{opacity}",
                    reference: node_ref,
                    position: "absolute",

                    label {
                        max_lines: "1",
                        width: "auto",
                        font_size: "14",
                        text_overflow: "clip",
                        color: "{THEME.on_primary_container}",

                        {props.label.as_str()}
                    }
                }
            }
        }
    }
}
